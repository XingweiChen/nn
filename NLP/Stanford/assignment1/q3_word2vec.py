...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    grad[target:target + 1] = (
        a_target - 1) * predicted  # gradient for target value
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    for context in contextWords:
        c, g_in, g_out = word2vecCostAndGradient(input_vector, tokens[context],
...
def cbow(currentWord,
    ...
    for contextWord in contextWords:
        ...
        c, g_in, g_out = word2vecCostAndGradient(
            input_vector, tokens[currentWord], outputVectors, dataset)
        cost += c
        gradIn[idx:idx + 1, :] += g_in
        gradOut += g_out
    ### END YOUR CODE
